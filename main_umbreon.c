...
int main(){
    ...
    umbreon_encrypt(c,m,mlen,ad,adlen,npub,k,adTweakey, msgTweakey);
    ...
    forkInvert(m_out,c_out,tag,msgTweakey,0,INV_BOTH);
    for(int i=0;i<8;i++){
        newMsg[i]=c_out[i]^ciphertext[i]; 
    }
    for(int i=0;i<8;i++){
        runningTag[i]=m_out[i]^newMsg[i];
    }

    forkInvert(newAD,c_out,runningTag,adTweakey,0,INV_BOTH);

    umbreon_encrypt(c2,newMsg, mlen,newAD,adlen,npub,k2,adTweakey,msgTweakey);
    ...
    // unsigned long long clen=sizeof(clen);
    ...
    // umbreon_decrypt(m,c,clen+8,ad,adlen,npub,k);
